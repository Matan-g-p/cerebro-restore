...
def generate_and_upload_heroes(**kwargs):
    ...
    s3 = get_minio_client()
    s3.put_object(Bucket=BUCKET_NAME, Key='heroes.csv', Body=csv_buffer.getvalue())
    print("Uploaded heroes.csv to MinIO")
...
def generate_and_upload_missions(**kwargs):
    ...
    s3 = get_minio_client()
    s3.put_object(Bucket=BUCKET_NAME, Key='mission_logs.json', Body=json.dumps(logs, indent=4))
    print("Uploaded mission_logs.json to MinIO")

def generate_and_upload_biometrics(**kwargs):
    """Generates biometrics and uploads to MinIO."""
    ti = kwargs['ti']
    hero_ids = ti.xcom_pull(task_ids='generate_heroes')
    print("Generating biometrics...")
    ...
    s3 = get_minio_client()
    s3.put_object(Bucket=BUCKET_NAME, Key='biometrics.parquet', Body=parquet_buffer.getvalue())
    print("Uploaded biometrics.parquet to MinIO")
